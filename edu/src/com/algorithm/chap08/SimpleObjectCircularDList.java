...
class SimpleObject3 {
	...
	public SimpleObject3(String sno, String sname) {
		this.sno = sno;
		this.sname = sname;
	}
...
class Node2 {
	...
	Node2(String sno, String sname) {
		data = new SimpleObject3(sno, sname);
		llink = rlink = this;
	}
	public int compareNode(Node2 n2) {
		SimpleObject3 so1 = this.data;
		if (SimpleObject3.NO_ORDER.compare(so1, n2.data) < 0) return -1;
		else if (SimpleObject3.NO_ORDER.compare(so1, n2.data) > 0)return 1;
		else return 0;
	}
}
...
// --- 노드를 검색 ---//
	public SimpleObject3 search(SimpleObject3 obj, Comparator<? super SimpleObject3> c) {
...
// --- 올림차순으로 정렬이 되도록 insert ---//
	public void add(SimpleObject3 obj, Comparator<? super SimpleObject3> c) {
...
// --- list에 삭제할 데이터가 있으면 해당 노드를 삭제 ---//
	public void delete(SimpleObject3 obj, Comparator<? super SimpleObject3> c) {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case Add: // 머리노드 삽입
				...
				sname1 = sc.next();
				SimpleObject3 so = new SimpleObject3(sno1, sname1);
				lst1.add(so, SimpleObject3.NO_ORDER);
				break;
			case Delete: // 머리 노드 삭제
				...
				System.out.println(" 회원이름: ");
				sname1 = sc2.next();
				SimpleObject3 so2 = new SimpleObject3(sno1, sname1);
				lst1.delete(so2, SimpleObject3.NO_ORDER);
